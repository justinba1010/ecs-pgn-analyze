...
class Stockfish:
    ...
    def __init__(
        ...
        if self.does_current_engine_version_have_wdl_option():
            self._set_option("UCI_ShowWDL", "true", True, True)
    ...
    def _set_option(
        self, name: str, value: Any, update_parameters_attribute: bool = True, sleep=False
    ) -> None:
        self._put(f"setoption name {name} value {value}")
        if update_parameters_attribute:
            self._parameters.update({name: value})
        self._is_ready(sleep)

    def _is_ready(self, sleep=False) -> None:
        self._put("isready")
        if sleep:
          print("Sleep")
          #time.sleep(1)
    ...
    def get_top_moves(self, num_top_moves: int = 5, time: int = 0) -> List[dict]:
